%============================= createFunction ============================
%
% @brief    Create interface function code needed during optimization.
%
%============================= createFunction ============================

%
% @file     createFunction.m
%
% @author   Raktim Bhattacharya,    raktim@aero.tamu.edu [original]
%           Patricio A. Vela,       pvela@gatech.edu
%
% @note
%   indent is 2 spaces.
%   tab aligned to 4 spaces, with conversion.
%
% Original notes/copyright:
%   OCP2NLP
%   Copyright (c) 2005 by
...
%
%============================= createFunction ============================
function Func = createFunction(OutputSequence,Cin,tag,ProbName,ParamList)
  ...
  if strcmp(Type,tag)
    if ischar(c.func)
      ...
      for j = 1:length(OutputSequence)
        ...
        if sum(I) == 0
          GRAD = sprintf('zeros(size(%s))',OutputSequence{j}); 
          ...
          if isempty(symvar(GRAD))
            GRAD = sprintf('%s*ones(size(%s))',GRAD,OutputSequence{j}); 
                % For vectorization purposes.
          end
        end
        switch typeCin
